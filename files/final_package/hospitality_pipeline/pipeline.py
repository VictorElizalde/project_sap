"""
pipeline.py — Hospitality Pipeline
====================================
Runs 4 queries and pushes to Power BI.

Cline: run `python main.py --dry-run` first to test connections.
Then `python main.py` for the full run.

Queries run:
  05_RAWventas.sql          → RAWventas_hospitality  (Hospitality canal filter applied here)
  02_RAWalbaranes.sql       → RAWalbaranes
  07_RAWclientes.sql        → RAWclientes
...
QUERIES = {
    "ventas":    ("queries/05_RAWventas.sql",    "RAWventas_hospitality", 50),
    "albaranes": ("queries/02_RAWalbaranes.sql", "RAWalbaranes",          100),
    "clientes":  ("queries/07_RAWclientes.sql",  "RAWclientes",           10),
    "familias":  ("queries/04_RAWfamilias.sql",  "RAWfamilias",           10),
}

# Hospitality canal filter — applied in Python after query runs
# so we don't need two versions of the SQL file
HOSPITALITY_FAMILIES = [
    'HOTEL TV', 'TV ACCESORIOS', 'INSTALACION', 'MANTENIMIENTO',
    'LICENCIAS', 'SERVICIOS', 'SERVICIOS PROPIOS', 'IT HOSPITALITY',
    'MONITORES', 'LED', 'VIDEOWALL', 'SOPORTES', 'GAMA BLANCA',
]
...
class Pipeline:
    ...
    def run_all(self):
        log.info("Running Hospitality pipeline (%d queries)", len(QUERIES))
        errors = []
        for name in QUERIES:
            try:
    ...
    def run_single(self, query_name: str):
        ...
        # Apply Hospitality canal filter to ventas
        if query_name == "ventas":
            before = len(df)
            if "Familia" in df.columns:
                df = df[df["Familia"].isin(HOSPITALITY_FAMILIES)]
            log.info("Hospitality filter: %d → %d rows", before, len(df))
    ...
    def _print_summary(self):
        log.info("=" * 50)
        log.info("HOSPITALITY PIPELINE SUMMARY")
        log.info("=" * 50)
